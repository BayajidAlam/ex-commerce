...
router.post('/', sellerAuth, [
  ...
  body('category').isIn(['casual', 'formal', 'traditional', 'bag', 'jewellry', 'glass', 'watch']).withMessage('Invalid category'),
...
router.put('/:id', sellerAuth, [
  body('name').optional().trim().notEmpty(),
  body('description').optional().trim().notEmpty(),
  body('price').optional().isFloat({ min: 0 }),
  body('category').optional().isIn(['casual', 'formal', 'traditional', 'bag', 'jewellry', 'glass', 'watch'])
], async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
